...
struct CoreParams
{
  ...
  {
    double eps = 1e-10;
    return this->look_back_n == other.look_back_n && this->stride == other.stride &&
           std::abs(this->sc_sim_score - other.sc_sim_score) < eps && std::abs(gamma - other.gamma) < eps;
  }
...
private:
  ...
  // publishers
  rclcpp::Publisher<pogolm_interfaces::msg::LoopEdge>::SharedPtr loop_edge_publisher;
  rclcpp::Publisher<pogolm_interfaces::msg::LoopDebug>::SharedPtr loop_debug_publisher;
  // subscriptions
  rclcpp::Subscription<factor_graph_interfaces::msg::FactorGraph>::SharedPtr pose_graph_subscriber;
  rclcpp::Subscription<pogolm_interfaces::msg::Key>::SharedPtr pc_key_subscriber;
  // periodic cleanup
  rclcpp::TimerBase::SharedPtr clean_timer;

  // mutex / thread
  std::atomic<uint64_t> last_key{ 0 }, last_pointcloud_inserted{ 0 }, last_loop_checked_key{ 0 };
  std::mutex pose_mutex, pc_mutex, sc_mutex, cache_mutex;
  ...
  std::array<CorePolicy, 4> core_policies = { { { 100, CoreParams{ -1, 1, 0.4 }, CoreParams{ -1, 1, 0.5 }, false },
                                                { 1000, CoreParams{ 100, 1, 0.4 }, CoreParams{ 100, 2, 0.5 }, true },
                                                { 3000, CoreParams{ 300, 3, 0.4 }, CoreParams{ 300, 5, 0.5 }, true },
                                                { 10000, CoreParams{ 300, 3, 0.4 }, CoreParams{ 300, 10, 0.5 },
                                                  true } } };
  ...
  std::unique_ptr<SC2::SCManager> sc_manager;
  ...
  DescriptorMatchResult descriptor_matching(const uint64_t target_key, const uint64_t source_key);

  // scan matching with initial guess
  ScanMatchResult scan_matching(const Eigen::Isometry3d& init_guess, const uint64_t target_key,
                                const uint64_t source_key);

  // scan matching using descriptor match result
  ScanMatchResult scan_matching(const DescriptorMatchResult& dm_result);

  // run VICP alignment
  void perform_icp_alignment(Eigen::Isometry3d& initial_guess, small_gicp::RegistrationResult& icp_reg_res,
                             const disk::VoxelmapCache& target, const disk::VoxelmapCache& source);

  // compute initial guess using KISS matcher
  void kiss_initial_guess(Eigen::Isometry3d& init_guess, long& kiss_inliers, const disk::VoxelmapCache& target,
  ...
  void publish_loop_debug(const uint64_t source_key, const std::vector<DescriptorMatchResult>& desc_matches,
...
public:
  ...
  LoopDetector(const LoopDetectorParams& params, const rclcpp::NodeOptions& options = rclcpp::NodeOptions());
...
{
public:
  LoopDetectorComponent(const rclcpp::NodeOptions& options) : LoopDetector(options)
  {
  }

  LoopDetectorComponent(const LoopDetectorParams& params, const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
