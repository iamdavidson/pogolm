...
struct Landmark
{
  ...
  Landmark() = default;
  Landmark(uint64_t _key, float _x, float _y, float _z, std::string _l) : point(_x, _y, _z), key(_key), label(_l)
  {
  }
};
...
inline gtsam::Pose3 to_pose3(const Eigen::Isometry3d& iso)
{
  ...
  const gtsam::Point3 t(iso.translation().x(), iso.translation().y(), iso.translation().z());
...
inline std::vector<Eigen::Vector3f> pc2_to_vec3f_fast(const sensor_msgs::msg::PointCloud2& pc)
{
...
inline bool calculate_cov_from_hessian(const Eigen::Matrix<double, 6, 6>& hessian,
									   const Eigen::Matrix<double, 6, 6>& fallback, Eigen::Matrix<double, 6, 6>& cov)
{
  cov = fallback;
...
inline uint64_t x_(uint64_t key)
{
  return gtsam::Symbol('x', key).key();
}
inline uint64_t l_(uint64_t key)
{
  return gtsam::Symbol('l', key).key();
}
...
inline uint64_t encode_keys(uint64_t pose_key, uint64_t landmark_key)
{
  ...
  {
	throw std::out_of_range("[ENCODE_KEYS] keys value exceed 32 bit: encoding does not work!");
  }
...
inline KD_TREE<LMPoint>::PointVector get_buffer(const std::map<uint64_t, std::shared_ptr<Landmark>>& p_map)
{
  KD_TREE<LMPoint>::PointVector v;
...
{
  return LMPoint{ static_cast<float>(p.x()), static_cast<float>(p.y()), static_cast<float>(p.z()), 0 };
}

// apply transform to point set (used for the initial guess of KISS-Matcher)
inline std::vector<Eigen::Vector3f> apply_transform(const std::vector<Eigen::Vector3f>& pts, const Eigen::Isometry3d& T)
{
...
static inline void appendPoseTxtEntry(const std::string& poses_txt_path, uint64_t key, const Eigen::Matrix4d& T)
{
  std::ofstream ofs(poses_txt_path, std::ios::out | std::ios::app);
  if (!ofs.is_open())
  {
	throw std::runtime_error("Could not open poses_txt_path for append: " + poses_txt_path);
  }
  ...
  for (int r = 0; r < 4; ++r)
  {
	ofs << "  " << std::setw(10) << T(r, 0) << "  " << std::setw(10) << T(r, 1) << "  " << std::setw(10) << T(r, 2)
		<< "  " << std::setw(10) << T(r, 3) << "\n";
  }
  ofs << "\n";
}
...
inline sensor_msgs::msg::PointCloud2 filter_xyz(const sensor_msgs::msg::PointCloud2& in)
{
  ...
  mod.setPointCloud2Fields(3, "x", 1, sensor_msgs::msg::PointField::FLOAT32, "y", 1,
...
public:
  ...
  static std::shared_ptr<disk::DiskCloudStore> get(const std::string& path, int zstd_level = 1,
												   bool truncate_existing = false, bool flush = false)
  {
	...
	auto sp = std::make_shared<disk::DiskCloudStore>(path, zstd_level, truncate_existing, flush);
	stores_[k] = sp;
	return sp;
  }
...
private:
  ...
  static inline std::unordered_map<Key, std::weak_ptr<disk::DiskCloudStore>, KeyHash> stores_;
};
...
public:
  ...
  LineWriter(const std::filesystem::path& filePath, Mode mode = Mode::Truncate)
  {
...
inline Eigen::Matrix<double, 6, 6> ros_to_gtsam_cov(const std::array<double, 36>& cov)
{
  Eigen::Matrix<double, 6, 6> cov_ros = Eigen::Map<const Eigen::Matrix<double, 6, 6, Eigen::RowMajor>>(cov.data());
...
static inline Eigen::Matrix<double, 6, 6> ros_twist_to_gtsam_increment_cov(const std::array<double, 36>& twist_cov_ros,
																		   double dt)
{
  Eigen::Matrix<double, 6, 6> C = Eigen::Map<const Eigen::Matrix<double, 6, 6, Eigen::RowMajor>>(twist_cov_ros.data());
