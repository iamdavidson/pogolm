...
void PoseGraph::callback_pose_add_timer() {
    ...
    if (!odom_available.load(std::memory_order_acquire) || \
        !lidar_available.load(std::memory_order_acquire)) return;
...
gtsam::Pose3 PoseGraph::get_current_pose() {
    ...
    if (!last_odom_msg) return gtsam::Pose3::Identity();
...
std::optional<uint64_t> PoseGraph::add_pose(const bool is_anchor) {
    ...
    double dt = 0.0;
    if (std::isfinite(prev_stamp_s)) dt = stamp_s - prev_stamp_s;
    if (dt <= 0.0) dt = 0.01;
    ...
    if (!is_anchor) {
...

        {
            ...
            if (has_pc) pc_stamp_s = rclcpp::Time(last_pc_msg->header.stamp).seconds();
        }
    ...
    gtsam::Pose3 raw_odom;
    {
        ...
        if (current_pose_key == STARTING_POSE_KEY) {
            ...
            if (!is_anchor && 
                !linear_crit(odom_edge) && 
                !angular_crit(odom_edge) && 
                !timeout_crit(odom_edge)) return std::nullopt; 
        ...
        if (pose_timestamps_.size() <= new_pose_key) pose_timestamps_.resize(new_pose_key + 1);
        pose_timestamps_[new_pose_key] = stamp_s;

        if (raw_odom_poses_.size() <= new_pose_key) raw_odom_poses_.resize(new_pose_key + 1);
    ...
    if (!is_anchor) {
        ...
        if (!pc_local) {
            LOGE_ANY(this->get_logger(),
                "No pointcloud available for key %lu -> skipping publish/save", new_pose_key);
        } else {
...
void PoseGraph::callback_loop_edge(const pogolm_interfaces::msg::LoopEdge& msg) {
    ...
    const Eigen::Matrix4d T_mat = Eigen::Map<const Eigen::Matrix<double, 4, 4, Eigen::RowMajor>>(msg.tranformation.data());
    ...
    auto robust = gtsam::noiseModel::Robust::Create(
        gtsam::noiseModel::mEstimator::Huber::Create(1.3),
        information
    );

    double loop_dist = T.translation().norm();

    if (verbose) LOGI_LOOP(this->get_logger(), "revieced <%lu,%lu> norm: %.4f",
            key_(msg.source_key), key_(msg.target_key), loop_dist);
...
void PoseGraph::optimization_worker() {
    ...
    params.linearSolverType 
        = gtsam::NonlinearOptimizerParams::LinearSolverType::MULTIFRONTAL_CHOLESKY;
    ...
    while (running) {
        ...
        for (const auto& cand: local_loop_edge_buffer) {
            ...
            pose_graph_local.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
                cand->source_key,
                cand->target_key,
                to_pose3(cand->transformation),
                cand->noise
            );
        ...
        if (verbose) LOGI_OPT(log, "================== Optimizing ==================");
...

        {
            ...
            for (const auto& cand: local_loop_edge_buffer) { loop_edge_tracker.emplace_back(cand); }
            ...
            for (const auto& v : optimized_values) {
                ...
                if (!point_v || !landmark_exists_unsafe(key)) continue;
...
void PoseGraph::print_cand_error(const std::shared_ptr<LoopEdgeCandidate>& cand, const gtsam::Values& values, bool verbose) {
...
void PoseGraph::fast_remove_factor(uint64_t factor_key) {
    ...
    if (factor_key >= pose_graph.size()) return;
...
std::vector<uint64_t> PoseGraph::add_landmark(
    const gtsam::Point3& p, /* in local coord system */
    ...
    uint64_t new_landmark_key;
    {
        ...
        if (landmark_data.size() % IKD_REBUILD_THRES == 0) rebuild_ikdtree();
        ...
        for (auto& pose_key : new_lm.anchor_list) {
            ...
            pose_graph.emplace_shared<gtsam::BearingRangeFactor<gtsam::Pose3, gtsam::Point3> >(
                x_(pose_key),
                l_(new_lm.key),
                bearing,
                range,
...
pogolm_interfaces::msg::Landmark PoseGraph::to_ros_landmark(const Landmark& lm) {
    ...
    for (auto k : lm.anchor_list) out.anchor_keys.push_back(k);

    return out;
}

void PoseGraph::handle_store_landmark(
    const std::shared_ptr<pogolm_interfaces::srv::StoreLandmark::Request> req,
...
void PoseGraph::handle_query_landmark(
    const std::shared_ptr<pogolm_interfaces::srv::QueryLandmark::Request> req,
    ...
    try {
        ...
        switch (req->mode) {
            ...
            case 5: { // KNN_BY_LANDMARK
                ...
                q.point = LMPoint(
                    static_cast<float>(req->landmark.position.x),
                    static_cast<float>(req->landmark.position.y),
                    static_cast<float>(req->landmark.position.z),
                    q.key
                );
...
bool PoseGraph::remove_landmark_unsafe (uint64_t query_landmark_key) {
    ...
    if (!node_exists_unsafe(l_(query_landmark_key)) ||
        !landmark_exists_unsafe(query_landmark_key)) return false;
...
void PoseGraph::rebuild_ikdtree() {
    ...
    if (landmark_data.size() == 0) return;

    auto buf = get_buffer(landmark_data); 
    lm_ikd_tree.Build(buf);
}

void PoseGraph::range_search (
    const float& radius,
    const Landmark& lm,
    SearchResult& result
) {
...
void PoseGraph::merge_neighbors(Landmark& lm, const SearchResult& result) {
    ...
    for (const auto& [n_lm_key, _] : result.dists) { 
        ...
        if (!node_exists_unsafe(l_(n_lm_key)) ||
            !landmark_exists_unsafe(n_lm_key)) continue;
...
PoseGraph::get_neighbors_range_by_lm(const float radius, const Landmark& query_lm) {
    ...
    if (res.dists.empty()) return {};

    ret.reserve(res.dists.size());
    for (const auto& [k, _] : res.dists) {
        auto it = landmark_data.find(k);
        if (it == landmark_data.end()) continue;
        ret.push_back(it->second);
    }
    return ret;
}

std::vector<std::shared_ptr<Landmark>> PoseGraph::get_neighbors_range_by_key (const float radius, const uint64_t query_key) {
...
std::vector<std::shared_ptr<Landmark>> PoseGraph::get_neighbors_range_by_label (const float radius, const std::string query_label) {
...
std::vector<std::shared_ptr<Landmark>>
PoseGraph::get_neighbors_knn_by_lm(const int k, const Landmark& query_lm) {
...
void PoseGraph::init_node(const PoseGraphParams& params) {
    ...
    odom_subscriber = this->create_subscription<nav_msgs::msg::Odometry>(
        odom_topic_name,
        10,
        std::bind(&PoseGraph::callback_odometry,this,std::placeholders::_1),
        io_opt
    ); 
    ...
    pc_subscriber = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        lidar_topic_name,
        10,
        std::bind(&PoseGraph::callback_pointcloud,this,std::placeholders::_1),
        io_opt
    );
    ...
    le_subscriber = this->create_subscription<pogolm_interfaces::msg::LoopEdge>(
        "/pogolm/loop_edge",
        10,
        std::bind(&PoseGraph::callback_loop_edge,this,std::placeholders::_1),
        io_opt
    );


    //POSE GRAPH GROUP
    pose_add_timer = this->create_wall_timer(
        50ms,
        std::bind(&PoseGraph::callback_pose_add_timer,this),
        pg_group
    );

    odom_timeout_timer = this->create_wall_timer(
        500ms,
        std::bind(&PoseGraph::callback_odom_timeout_timer, this),
        pg_group
    );

    // PUBLISHER
    fg_publisher = this->create_publisher<factor_graph_interfaces::msg::FactorGraph>(
        "/pogolm/pose_graph",10
    );

    pc_key_publisher = this->create_publisher<pogolm_interfaces::msg::Key>(
    ...
    merge_info_pub = this->create_publisher<pogolm_interfaces::msg::MergedInfo>(
        "/pogolm/merge_info", 10
    );

    loop_tracker_pub = this->create_publisher<pogolm_interfaces::msg::LoopEdgeTracker>(
        "/pogolm/loop_edge_tracker", 10
    );

    // SERVICES
    store_landmark_srv = this->create_service<pogolm_interfaces::srv::StoreLandmark>(
        "/pogolm/store_landmark",
        std::bind(&PoseGraph::handle_store_landmark, this,
                  std::placeholders::_1, std::placeholders::_2)
    );

    query_landmark_srv = this->create_service<pogolm_interfaces::srv::QueryLandmark>(
        "/pogolm/query_landmark",
        std::bind(&PoseGraph::handle_query_landmark, this,
                  std::placeholders::_1, std::placeholders::_2)
    );
...
*/
PoseGraph::PoseGraph(
    const PoseGraphParams& params,
    const rclcpp::NodeOptions& options
):Node("pose_graph",options)  {
