...
void LoopDetector::callback_cleaner() {
    ...
    if (verbose) LOGI_CLEANER(this->get_logger(), " %d MB RAM (trimmed<%s> -> omitted %d MB RAM)", after,(trimmed)?"true":"false", std::abs(before-after));
}
...
void LoopDetector::update_core_policy(const size_t n) {
    ...
    if (enabled) far.cv.notify_all();
    if (updated) {
        LOGI_POLI(this->get_logger(), "Updated core policy (%ld) for |graph|=%lu (LBN=%d)", current_policy, n, policy->near.look_back_n);
    }
} 
...
void LoopDetector::core_worker(CoreContext& core) { 
    ...
    while (core.running.load()) {
        ...
        uint64_t source_key;
        {
            ...
            core.cv.wait(lock,[&] { 
                return (core.enabled.load() && !core.jobs.empty()) || !core.running.load(); 
            });

            if (!core.running.load()) break;
        ...
        for (uint64_t target_key=start_key; target_key>=end_key; target_key -= params.stride) {
            ...
            if (target_key<end_key || target_key>start_key) break; 
            // if (target_key>start_key) break; 

            if (!pointcloud_exists(target_key)) continue;

            uint64_t diff = (source_key>target_key)? 
        ...
        auto dm_pipeline_ms =  std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(std::chrono::steady_clock::now() - t0).count();
        ...
        for (size_t i=0; i<check_max; i++) {
            ...
            if (work_time_ms >= RT_LIMIT_MS) break;
        ...
        last_loop_checked_key.store(
            std::max(source_key, last_loop_checked_key.load(std::memory_order_relaxed)),
...
LoopDetector::descriptor_matching (
...
ScanMatchResult
LoopDetector::scan_matching(const DescriptorMatchResult& dm_result) {
    ...
    if(target_ret)  target_cache = *target_ret;
    else            target_cache = build_voxelmap(dm_result.target_key);

    if(source_ret)  source_cache = *source_ret;
    else            source_cache = build_voxelmap(dm_result.source_key);
    ...
    if (sm_result.kiss_converged) {
        ...
        sm_result.icp_duration_ms =     std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t_1_s - t_0_s).count();
    } else {
        sm_result.icp_result.converged = false;
    }
    ...
    sm_result.kiss_duration_ms =    std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t_1_k - t_0_k).count();
    sm_result.rmse_duration_ms =    std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t_1_r - t_0_r).count();
    sm_result.fetch_duration_ms =   std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t_1_f - t_0_f).count();
...
void LoopDetector::perform_icp_alignment(
    ...
    static thread_local std::unique_ptr<small_gicp::Registration<small_gicp::GICPFactor, small_gicp::ParallelReductionOMP>>
        reg;
    ...
    const auto icp_result = reg->align(
        *target.vm_sg,
        *source.pc_sg,
        *target.vm_sg,
        initial_guess
...
void LoopDetector::kiss_initial_guess(
...
std::shared_ptr<disk::VoxelmapCache> LoopDetector::build_voxelmap(const uint64_t key) {
    ...
    auto kdtree = std::make_shared<small_gicp::KdTree<small_gicp::PointCloud>>(cache->pc_sg , small_gicp::KdTreeBuilderOMP(NUM_TH/ 2));       
...
bool LoopDetector::verify_loop (ScanMatchResult cand) {
    ...
    if (cand.icp_result.converged && 
        cand.kiss_converged &&
        cand.icp_result.num_inliers >= ICP_INLIER_TRES) {

        num_loops_accepted++;
        if (cand.core==idx(NEAR)) num_near++;
        else num_far++;
        ...
        if (verbose) {
            ...
            LOGI_VERIFY_OK(this->get_logger(), "___________ VERIFIED LOOP CANDIDATE <%lu>---<%lu>", cand.target_key,cand.source_key);
            LOGI_VERIFY_OK(this->get_logger(), "___________ [kiss]––––––––––inlier-----------: %ld >= %d", cand.kiss_inliers, KISS_INLIER_TRES);
            LOGI_VERIFY_OK(this->get_logger(), "___________ [small_gicp]––––inlier------------: %ld >= %d", cand.icp_result.num_inliers, ICP_INLIER_TRES);
            LOGI_VERIFY_OK(this->get_logger(), "–––––––––––––––––––––––––––––––––––––––––––––––––––––––––");
        ...
        if (verbose) {
            ...
            LOGW_VERIFY_NO(this->get_logger(), "___________ OMITTED LOOP CANDIDATE <%lu>---<%lu>", cand.target_key,cand.source_key);
            LOGW_VERIFY_NO(this->get_logger(), "___________ [kiss]––––––––––inlier-----------: %ld <= %d", cand.kiss_inliers, KISS_INLIER_TRES);
            LOGW_VERIFY_NO(this->get_logger(), "___________ [small_gicp]––––inlier------------: %ld <= %d", cand.icp_result.num_inliers, ICP_INLIER_TRES);
            LOGW_VERIFY_NO(this->get_logger(), "–––––––––––––––––––––––––––––––––––––––––––––––––––––––––");
...
void LoopDetector::publish_loop_debug (
    ...
    for (const auto& m : desc_matches) {
        if (m.source_key != source_key) continue;
    ...
    for (const auto& s : scan_matches) {
        if (s.source_key != source_key) continue;
...
void LoopDetector::init_node(const LoopDetectorParams& params) {
    ...
    vm_storage = std::make_shared<disk::DiskVoxelmapCacheStore>("/tmp/vm_store.bin", 1, /*truncate*/true, /*flush*/false);
    ...
    for (size_t i=0; i<core_policies.size(); i++) {
        ...
        LOGI_POGOLM(this->get_logger(), "–– [%ld,%ld] N (lbn:%d, s:%d, sc:%.2f, g:%.1f) F (lbn:%d, s:%d, sc:%.2f, g:%.1f) %s", 
            begin,
            core_policies[i].thres,
            core_policies[i].near.look_back_n,
            core_policies[i].near.stride,
            core_policies[i].near.sc_sim_score,
            core_policies[i].near.gamma,
            core_policies[i].far.look_back_n,
            core_policies[i].far.stride,
            core_policies[i].far.sc_sim_score,
            core_policies[i].far.gamma,
    ...
    cores[idx(NEAR)].thd 
        = std::thread(&LoopDetector::core_worker, this, std::ref(cores[idx(NEAR)]));
    cores[idx(FAR)].thd 
        = std::thread(&LoopDetector::core_worker, this, std::ref(cores[idx(FAR)]));

    // scancontext++
    sc_manager = std::make_unique<SC2::SCManager>(
        LIDAR_H,NUM_RING,NUM_SECT,MAX_RAD,
        EXCLUDE_RECENT,NUM_CAND,SEARCH_RATIO,
        SC_DIST_TRES,TREE_MAKING_P,SC_D_RES
    );
    ...
    pose_graph_subscriber = this->create_subscription<factor_graph_interfaces::msg::FactorGraph>(
        "/pogolm/pose_graph",
        10,
        std::bind(&LoopDetector::callback_pose_graph,this, std::placeholders::_1),
        io_opt
    );  

    pc_key_subscriber = this->create_subscription<pogolm_interfaces::msg::Key>(
        "/pogolm/pointcloud_key",
        10,
        std::bind(&LoopDetector::callback_pc_key, this, std::placeholders::_1),
        io_opt
    );

    loop_edge_publisher = this->create_publisher<pogolm_interfaces::msg::LoopEdge>(
        "/pogolm/loop_edge",
        10
    );


    clean_timer = this->create_wall_timer(
        std::chrono::seconds(10),
        std::bind(&LoopDetector::callback_cleaner,this)
    );
...
:Node("loop_detector",options) {
    RCLCPP_INFO(this->get_logger(), "IPC: %s",
...
LoopDetector::LoopDetector(const rclcpp::NodeOptions& options)
:Node("loop_detector",options) {
...
LoopDetector::~LoopDetector() {
    ...
    for (auto& core : cores) {
        if (core.thd.joinable()) core.thd.join();
    }
    ...
    auto percentile_sorted = [](const std::vector<double>& v_sorted, double p) -> double {
        if (v_sorted.empty()) return std::numeric_limits<double>::quiet_NaN();
