...
void MapModule::handle_map_request(
    ...
    LOGI_LIDAR(this->get_logger(),
        "map request: %lu poses, %lu landmarks — fetching pointclouds from disk",
        n_poses, n_landmarks);
    ...
    LOGI_LIDAR(this->get_logger(),
        "map request done: %lu clouds fetched, %lu not found on disk, %lu landmarks",
        fetched, not_found, n_landmarks);
}


std::optional<sensor_msgs::msg::PointCloud2>
MapModule::fetch_pointcloud(uint64_t key) {
...
std::optional<sensor_msgs::msg::PointCloud2>
MapModule::get_pointcloud(uint64_t key) {
...
MapModule::MapModule(const rclcpp::NodeOptions& options)
: rclcpp::Node("map_module", options) {
    ...
    pose_graph_subscriber = this->create_subscription<factor_graph_interfaces::msg::FactorGraph>(
        "pose_graph",
        10,
        std::bind(&MapModule::callback_pose_graph, this, std::placeholders::_1)
    );

    get_map_srv = this->create_service<pogolm_interfaces::srv::GetMap>(
        "get_map",
        std::bind(
            &MapModule::handle_map_request, this,
            std::placeholders::_1,
            std::placeholders::_2
        )
    );
